(function () {
  ...
  function buildCAGEDShapeSVG(shape, root, mode) {
    ...
    const W = 734; // fixed so all CAGED shapes render at the same height (70 + 4*160 + 24)
    const fretW = (W - padL - padR) / frets;
    const rowH = (H - padT - padB) / 5;
    const openX = padL - 52;

    const p = [];
    p.push(`<svg viewBox="0 0 ${W} ${H}" style="display:block;margin:0 auto;max-width:${W}px;width:100%" xmlns="http://www.w3.org/2000/svg">`);
    p.push(`<defs>
    <linearGradient id="fb-grad" x1="0" x2="0" y1="0" y2="1">
      <stop offset="0" stop-color="${t.fbTop}"/>
      <stop offset="1" stop-color="${t.fbBot}"/>
